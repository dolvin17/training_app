...
import { EJERCICIOS_CONFIG } from "@/app/page"; // Importas la lista
...
export default function GymApp({
  ...
  const ejercicioActual = EJERCICIOS_CONFIG.find(ex => ex.slug === slug);
  const nombreParaMostrar = ejercicioActual ? ejercicioActual.nombre : slug;

  // Ahora usas 'nombreParaMostrar' en el <h1> y en 'saveSerie'
  // Y usas 'slug' para la imagen: path={`${slug}.png`}
  // Convertimos el slug a formato legible (ej: "Crunch Abdominal")
  
  const nombreEjercicio = slug
    .replace(/-/g, " ")
  ...
  const cargarDatos = async () => {
    try {
      const data = await getHistorial(nombreEjercicio);
      setHistorial(data);
    } catch (e) {
  ...
  const manejarNuevaSerie = async (nuevaSerie: Partial<SerieEntrenamiento>) => {
    try {
      await saveSerie({ ...nuevaSerie, nombre_ejercicio: nombreEjercicio });
      cargarDatos();
  ...
  return (
    <div className="min-h-screen bg-black text-white p-6 max-w-md mx-auto">
      ...
      <div className="mb-8">
        {/* Aquí puedes usar el slug para cargar la imagen correcta automáticamente */}
        <ExerciseImage path={`${slug}.png`} alt={nombreEjercicio} />
      </div>
      ...
      <div className="text-center mb-6">
        <p className="text-gray-500 text-[10px] uppercase tracking-widest border-b border-white/5 pb-2 inline-block px-4">
          Último peso:{" "}
          <span className="text-white font-bold ml-1">{pesoAnterior} kg</span>
        </p>
      </div>
      ...
      {/* 7. Listado Histórico */}
      <div className="mt-4">
        <h2 className="text-gray-500 text-xs uppercase mb-4 tracking-tighter">
          Historial de hoy
        </h2>
