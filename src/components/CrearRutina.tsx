...
export default function CrearRutina({ ejerciciosDB, rutinaExistente }: any) {
  ...
  const generateEmptySessions = () => {
    ...
    const config = Array.from({ length: rutina.diasActivos }).map((_, i) => ({
      nombreSesion: "", // <--- Cambiado de `Nombre ${i+1}` a vacío
      ejercicios: [],
    }));
  ...
  const handleFinalizar = async () => {
    try {
      ...
      const slugFinal =
        rutina.slug ||
        rutina.nombrePlan.toLowerCase().trim().replace(/\s+/g, "-");
  ...
  return (
    <div className="min-h-screen bg-black text-white p-4 sm:p-6 w-full max-w-full md:max-w-2xl mx-auto">
      <header className="flex items-center gap-4 mb-10 mt-4">
        <button
          onClick={() =>
            paso === 2 && !rutinaExistente ? setPaso(1) : router.back()
          }
        ...
        <h1 className="text-xl font-black uppercase italic tracking-tighter">
          {rutinaExistente ? "Modificar" : "Diseñar"}{" "}
          <span className="text-orange-400">Rutina</span>
        </h1>
      </header>
        ...
        <div className="space-y-10 pb-44 animate-in fade-in slide-in-from-bottom-6 duration-700">
          <div className="px-2 space-y-4 mb-12">
            <div className="flex items-center gap-3">
              ...
              <label className="text-[10px] font-black uppercase text-zinc-500 tracking-[0.4em]">
                Nombre del Plan
              </label>
            </div>
            <input
              ...
              onChange={(e) =>
                setRutina({ ...rutina, nombrePlan: e.target.value })
              }
          ...
          <div className="space-y-10">
            {rutina.configuracion.map((s: any, i: number) => (
              <CardSesion
                ...
                onDelete={() => {
                  const nuevaConfig = rutina.configuracion.filter(
                    (_: any, idx: number) => idx !== i
                  );
                  setRutina({
                    ...rutina,
                    configuracion: nuevaConfig,
                    diasActivos: nuevaConfig.length,
                  });
                }}
              />
            ))}
            ...
            <button
              onClick={() => {
                const nuevaSesion = { nombreSesion: "", ejercicios: [] };
                setRutina({
                  ...rutina,
